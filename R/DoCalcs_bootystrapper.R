doCalcs <- function(crt, reaches=6, species, ...){
...
## weird, should round things after all the calculations. should check if it makes any difference if no rounding (8/9/2017)
if(species!= 'CH') {c0adults<- round(crt$C0adultj_rtn,0)}
else {c0adults<- round(crt$C0adult_rtn,0)}

if(species!= 'CH') {c0adults.<- round(crt$C0adultj_t_rtn,0)}
else {c0adults.<- round(crt$C0adult_t_rtn,0)}

if(species!= 'CH') {c1adults.<- round(crt$C1adultj_rtn,0)}
else {c1adults.<- round(crt$C1adult_rtn,0)}

if(species!= 'CH'){t0adults.<- round(crt$T0adultj_rtn,0)}
else {t0adults.<- round(crt$T0adult_rtn,0)}

if(species!= 'CH'){t1adults.<- round(crt$Txadultj_rtn,0)}
else{t1adults.<- round(crt$Txadult_rtn,0)}
...
if(species!= 'CH') {c0adults_b.<- round(crt$C0adultj_t_boa,0)}
else {c0adults_b.  <- round(crt$C0adult_t_boa,0)}

if(species!= 'CH') {c1adults_b.<- round(crt$C1adultj_boa,0)}
else {c1adults_b.<- round(crt$C1adult_boa,0)}

if(species!= 'CH') {t0adults_b.<- round(crt$T0adultj_boa,0)}
else {t0adults_b.<- round(crt$T0adult_boa,0)}
...
if(species!= 'CH') {totaladult.<- round((crt$C0adultj_t_rtn + crt$C1adultj_rtn + crt$Txadultj_rtn),0)}
else {totaladult.<- round((crt$C0adult_t_rtn + crt$C1adult_rtn + crt$Txadult_rtn),0)}
totaladult_gj.<- round((crt$C0adultj_t_rtn + crt$C1adultj_rtn + crt$Txadultj_rtn),0)

## deal with steelhead and sockeye not having 'jacks'
if(species!= 'CH') {totaladult_b.<- round((crt$C0adultj_t_boa + crt$C1adultj_boa + crt$Txadultj_boa),0)}
...
if(species!= 'CH') {lgsadults.<- round(crt$lgsadultj_rtn,0)}
else {lgsadults.<- round(crt$lgsadult_rtn,0)}

if(species!= 'CH') {lmnadults.<- round(crt$lmnadultj_rtn,0)}
else{lmnadults.<- round(crt$lmnadult_rtn,0)}
# ----
...
# c1_cjsNEW     <- R1. * s1_cjs * (p2 + (1-p2)*p3 + (1-p2)*(1-p3)*p4) - (delta2. + delta3./s2_cjs + delta4./(s2_cjs*s3_cjs) + delta1)
