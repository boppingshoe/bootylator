...
To summarize and export the results, use `doCalcs()` for Snake River or `doCalcs_uc()` for upper Columbia. Bootstrap results are saved in CSSOUTPUT in SQL server (PITTAG_2016), append parameter output in CSSREPORT in the same SQL server, and parameter output in csv file in current working directory. Exporting results is suppressed for the example below.
